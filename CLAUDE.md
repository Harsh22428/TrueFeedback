...
1. Test authentication flows (sign up → verify → sign in)
2. Test protected routes with/without auth
3. Verify database operations don't create duplicates
4. Check responsive design on mobile viewports
5. Test form validations with invalid inputs
...
**Solution**: Connection caching is implemented in `dbConnect.ts`

### Middleware Redirects
**Issue**: Infinite redirect loops
**Solution**: Check middleware matcher paths and auth state logic

### Email Verification
**Issue**: Verification codes expiring
**Solution**: Check `verifyCodeExpiry` is set correctly (usually 1 hour)

### Build Errors
**Issue**: Linting errors during build
**Solution**: Build uses `--no-lint` flag; run `npm run lint` separately

---

## Project-Specific Notes

1. **Model Export Pattern**: Uses `mongoose.models.User || mongoose.model()` to prevent recompilation errors in development

2. **Route Groups**: Uses `(app)` and `(auth)` groups for logical separation without affecting URLs

3. **AI Features**: Message suggestion endpoint uses Edge runtime for better performance

4. **Email Templates**: Located in `emails/` directory, using `@react-email/components`

5. **UI Library**: Based on shadcn/ui, customizable through `components.json`

6. **Theme Support**: Dark mode ready with `next-themes` integration
...
- Current branch pattern: `claude/claude-md-*`
...
- **Node Version**: Compatible with ES2017+
- **TypeScript Config**: `tsconfig.json`
- **Tailwind Config**: Uses Tailwind v4 with PostCSS
